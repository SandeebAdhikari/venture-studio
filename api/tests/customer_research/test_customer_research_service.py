"""Integration tests for customer research service."""
...
from app.db.enums import CategoryKind, CustomerResearchStatus, SourceType
...
@pytest.fixture
def customer_research_settings() -> Settings:
    return Settings(
        api_key="test-api-key-for-customer-research",
...
async def _create_opportunity(
    ...
    source = Source(
        name=f"customer-research-source-{uuid4()}",
...
@pytest.mark.asyncio
async def test_research_opportunity_persists_scores_and_evidence(
    db_session: AsyncSession,
    taxonomy_ids: tuple[UUID, UUID, UUID],
    ...
    assert result.customer_research_id is not None
    assert result.draft is not None
    assert result.draft.pain_score == 82
    assert result.draft.cares_verdict == "yes"
    assert result.draft.validation_metrics["validation_readiness_score"] > 0

    research = await repos.customer_research.get_by_id_with_evidence(result.customer_research_id)
    assert research is not None
    assert research.status == CustomerResearchStatus.COMPLETED.value
    assert research.is_current is True
    assert len(research.evidence) == 3
    assert research.evidence[0].complaint_id is not None
    assert len(research.representative_complaints) == 1
...
async def test_research_retries_malformed_response(
    ...
    opportunity = await _create_opportunity(db_session, taxonomy_ids)
    repos = get_repositories(db_session)
    mock = MockCustomerResearchLLMClient([None, default_mock_customer_research_output()])
    service = CustomerResearchService(repos, customer_research_settings, llm_client=mock)
    ...
    total_research = await db_session.scalar(select(func.count()).select_from(CustomerResearch))
    total_evidence = await db_session.scalar(
        select(func.count()).select_from(CustomerResearchEvidence)
    )
